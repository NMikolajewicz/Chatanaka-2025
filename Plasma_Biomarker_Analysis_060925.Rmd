...
# Helper functions for 95% CI reporting across binary outcomes
exact_binom_ci <- function(successes, total, conf.level = 0.95) {
  if (is.na(total) || total <= 0 || is.na(successes)) {
    return(c(estimate = NA_real_, lower = NA_real_, upper = NA_real_))
  }
  bt <- stats::binom.test(as.integer(successes), as.integer(total), conf.level = conf.level)
  c(
    estimate = as.numeric(bt$estimate),
    lower = as.numeric(bt$conf.int[1]),
    upper = as.numeric(bt$conf.int[2])
  )
}

extract_binary_counts <- function(cm_table, positive = "GBM") {
  table_mat <- as.matrix(cm_table)
  neg_levels <- setdiff(colnames(table_mat), positive)
  negative <- neg_levels[1]
  
  tp <- as.numeric(table_mat[positive, positive])
  fp <- as.numeric(table_mat[positive, negative])
  fn <- as.numeric(table_mat[negative, positive])
  tn <- as.numeric(table_mat[negative, negative])
  
  c(tp = tp, fp = fp, fn = fn, tn = tn, total = tp + fp + fn + tn)
}

bootstrap_f1_ci <- function(predicted, actual, positive = "GBM", conf.level = 0.95,
                            n_boot = 1000, seed = 123) {
  preds <- as.character(predicted)
  obs <- as.character(actual)
  n <- length(obs)
  negative <- setdiff(unique(c(obs, preds)), positive)[1]
  
  if (is.na(negative) || n == 0) {
    return(c(lower = NA_real_, upper = NA_real_))
  }
  
  f1_from_vectors <- function(pred_vec, obs_vec) {
    tp <- sum(pred_vec == positive & obs_vec == positive, na.rm = TRUE)
    fp <- sum(pred_vec == positive & obs_vec == negative, na.rm = TRUE)
    fn <- sum(pred_vec == negative & obs_vec == positive, na.rm = TRUE)
    denom <- (2 * tp) + fp + fn
    if (denom == 0) {
      return(NA_real_)
    }
  ...
  boot_vals <- replicate(n_boot, {
    ...
    f1_from_vectors(preds[idx], obs[idx])
  })
  
  alpha <- (1 - conf.level) / 2
  c(
    lower = as.numeric(stats::quantile(boot_vals, probs = alpha, na.rm = TRUE)),
    upper = as.numeric(stats::quantile(boot_vals, probs = 1 - alpha, na.rm = TRUE))
  )
}

safe_ci_auc <- function(roc_obj, conf.level = 0.95) {
  out <- tryCatch(
    as.numeric(pROC::ci.auc(roc_obj, conf.level = conf.level)),
    error = function(e) c(NA_real_, NA_real_, NA_real_)
  )
  c(lower = out[1], estimate = out[2], upper = out[3])
}

build_roc_ci_df <- function(roc_obj, model_name, conf.level = 0.95, boot.n = 1000, grid_n = 101) {
  specificities <- seq(1, 0, length.out = grid_n)
  
  to_numeric <- function(x, preferred_col = NULL) {
    if (is.null(x)) {
      return(numeric(0))
    }
    if (is.atomic(x)) {
    ...
    if (is.matrix(x) || is.data.frame(x)) {
      if (!is.null(preferred_col) && preferred_col %in% colnames(x)) {
        return(as.numeric(x[, preferred_col]))
      }
      return(as.numeric(x[, 1]))
    }
    if (is.list(x)) {
      if (!is.null(preferred_col) && !is.null(x[[preferred_col]])) {
        return(as.numeric(unlist(x[[preferred_col]], use.names = FALSE)))
      }
  ...
  tpr <- to_numeric(coords_obj, preferred_col = "sensitivity")
  if (length(tpr) != length(specificities)) {
    tpr <- rep(NA_real_, length(specificities))
  }
  
  ci_mat <- tryCatch(
    pROC::ci.se(roc_obj, specificities = specificities, conf.level = conf.level, boot.n = boot.n),
    error = function(e) NULL
  )
  ...
  ci_lower <- to_numeric(ci_df[, 1])
  ci_upper <- to_numeric(ci_df[, ncol(ci_df)])
  if (length(ci_lower) != length(tpr)) {
    ci_lower <- tpr
  }
  if (length(ci_upper) != length(tpr)) {
    ci_upper <- tpr
  }
  ...
  tibble(
    ...
    fpr = 1 - specificities,
    tpr = tpr,
    tpr_lower = pmax(0, ci_lower),
    tpr_upper = pmin(1, ci_upper)
  )
}

format_ci_text <- function(estimate, lower, upper, digits = 3) {
...
summarize_resample_ci <- function(df_model_long, conf.level = 0.95) {
  alpha <- (1 - conf.level) / 2
  df_model_long %>%
    dplyr::group_by(name) %>%
    dplyr::summarize(
      estimate = median(value, na.rm = TRUE),
      lower = as.numeric(stats::quantile(value, probs = alpha, na.rm = TRUE)),
      upper = as.numeric(stats::quantile(value, probs = 1 - alpha, na.rm = TRUE)),
      .groups = "drop"
    )
}
...
cv_roc_summary <- summarize_resample_ci(df.model.roc.long) %>% dplyr::mutate(metric = "ROC")
cv_sens_summary <- summarize_resample_ci(df.model.sens.long) %>% dplyr::mutate(metric = "Sensitivity")
cv_spec_summary <- summarize_resample_ci(df.model.spec.long) %>% dplyr::mutate(metric = "Specificity")

cv_metric_summary <- bind_rows(cv_roc_summary, cv_sens_summary, cv_spec_summary) %>%
  dplyr::mutate(
    CI_95 = purrr::pmap_chr(
      list(estimate, lower, upper),
      ~ format_ci_text(..1, ..2, ..3)
    )
  ) %>%
  dplyr::arrange(metric, dplyr::desc(estimate))

print("Cross-validated binary classification metrics with 95% CIs:")
print(cv_metric_summary %>% dplyr::select(name, metric, estimate, lower, upper, CI_95))
...
best_cv_roc <- cv_roc_summary %>% dplyr::filter(make.names(name) == make.names(best_model_name))
best_cv_sens <- cv_sens_summary %>% dplyr::filter(make.names(name) == make.names(best_model_name))
best_cv_spec <- cv_spec_summary %>% dplyr::filter(make.names(name) == make.names(best_model_name))

print(paste("Best performing model:", best_model_name))
print(paste(
  "Cross-validated AUC:",
  format_ci_text(best_cv_roc$estimate[1], best_cv_roc$lower[1], best_cv_roc$upper[1])
))
print(paste(
  "Cross-validated Sensitivity:",
  format_ci_text(best_cv_sens$estimate[1], best_cv_sens$lower[1], best_cv_sens$upper[1])
))
print(paste(
  "Cross-validated Specificity:",
  format_ci_text(best_cv_spec$estimate[1], best_cv_spec$lower[1], best_cv_spec$upper[1])
))
...
calculate_metrics <- function(pred_list, positive_class = "GBM", conf.level = 0.95) {
  
  # Confusion Matrix
  cm <- caret::confusionMatrix(pred_list$predictions, pred_list$actual, positive = positive_class)
  counts <- extract_binary_counts(cm$table, positive = positive_class)
  
  # ROC curve
  roc_obj <- pROC::roc(
    pred_list$actual,
    pred_list$probabilities[[positive_class]],
    levels = c(setdiff(levels(pred_list$actual), positive_class), positive_class),
    quiet = TRUE
  )
  auc_ci <- safe_ci_auc(roc_obj, conf.level = conf.level)
  
  f1_ci <- bootstrap_f1_ci(
    predicted = pred_list$predictions,
  ...
  # Metrics
  metrics <- list(
    accuracy = exact_binom_ci(counts["tp"] + counts["tn"], counts["total"], conf.level = conf.level),
    sensitivity = exact_binom_ci(counts["tp"], counts["tp"] + counts["fn"], conf.level = conf.level),
    specificity = exact_binom_ci(counts["tn"], counts["tn"] + counts["fp"], conf.level = conf.level),
    ppv = exact_binom_ci(counts["tp"], counts["tp"] + counts["fp"], conf.level = conf.level),
    npv = exact_binom_ci(counts["tn"], counts["tn"] + counts["fn"], conf.level = conf.level),
    f1 = c(
      estimate = as.numeric(cm$byClass["F1"]),
      lower = as.numeric(f1_ci["lower"]),
      upper = as.numeric(f1_ci["upper"])
    ),
    auc = c(
      estimate = as.numeric(auc(roc_obj)),
      lower = as.numeric(auc_ci["lower"]),
      upper = as.numeric(auc_ci["upper"])
    ),
...
performance_summary <- imap_dfr(test_metrics, ~{
  tibble(
    Model = .y,
    Accuracy = as.numeric(.x$accuracy["estimate"]),
    Accuracy_lower = as.numeric(.x$accuracy["lower"]),
    Accuracy_upper = as.numeric(.x$accuracy["upper"]),
    Sensitivity = as.numeric(.x$sensitivity["estimate"]),
    Sensitivity_lower = as.numeric(.x$sensitivity["lower"]),
    Sensitivity_upper = as.numeric(.x$sensitivity["upper"]),
    Specificity = as.numeric(.x$specificity["estimate"]),
    Specificity_lower = as.numeric(.x$specificity["lower"]),
    Specificity_upper = as.numeric(.x$specificity["upper"]),
    PPV = as.numeric(.x$ppv["estimate"]),
    PPV_lower = as.numeric(.x$ppv["lower"]),
    PPV_upper = as.numeric(.x$ppv["upper"]),
    NPV = as.numeric(.x$npv["estimate"]),
    NPV_lower = as.numeric(.x$npv["lower"]),
    NPV_upper = as.numeric(.x$npv["upper"]),
    F1 = as.numeric(.x$f1["estimate"]),
    F1_lower = as.numeric(.x$f1["lower"]),
    F1_upper = as.numeric(.x$f1["upper"]),
    AUC = as.numeric(.x$auc["estimate"]),
    AUC_lower = as.numeric(.x$auc["lower"]),
    AUC_upper = as.numeric(.x$auc["upper"])
  )
}) %>%
  dplyr::mutate(
    Accuracy_CI95 = purrr::pmap_chr(list(Accuracy, Accuracy_lower, Accuracy_upper), ~ format_ci_text(..1, ..2, ..3)),
    Sensitivity_CI95 = purrr::pmap_chr(list(Sensitivity, Sensitivity_lower, Sensitivity_upper), ~ format_ci_text(..1, ..2, ..3)),
    Specificity_CI95 = purrr::pmap_chr(list(Specificity, Specificity_lower, Specificity_upper), ~ format_ci_text(..1, ..2, ..3)),
    PPV_CI95 = purrr::pmap_chr(list(PPV, PPV_lower, PPV_upper), ~ format_ci_text(..1, ..2, ..3)),
    NPV_CI95 = purrr::pmap_chr(list(NPV, NPV_lower, NPV_upper), ~ format_ci_text(..1, ..2, ..3)),
    F1_CI95 = purrr::pmap_chr(list(F1, F1_lower, F1_upper), ~ format_ci_text(..1, ..2, ..3)),
    AUC_CI95 = purrr::pmap_chr(list(AUC, AUC_lower, AUC_upper), ~ format_ci_text(..1, ..2, ..3))
  )
...
# Build ROC curve data with pointwise 95% CI bands for each model
roc_ci_df <- imap_dfr(
  test_metrics,
  ~ build_roc_ci_df(.x$roc, .y, conf.level = 0.95, boot.n = 1000, grid_n = 101)
)
...
auc_df <- imap_dfr(
  ...
  mutate(
    AUC_CI95 = purrr::pmap_chr(list(AUC, AUC_lower, AUC_upper), ~ format_ci_text(..1, ..2, ..3)),
    model_label = paste0(model, " (", AUC_CI95, ")")
  )

label_map <- setNames(auc_df$model_label, auc_df$model)

print("Test-set AUROC estimates with 95% CIs:")
print(auc_df %>% dplyr::select(model, AUC, AUC_lower, AUC_upper, AUC_CI95))

ggplot(roc_ci_df, aes(x = fpr, y = tpr, color = model, fill = model)) +
  geom_ribbon(aes(ymin = tpr_lower, ymax = tpr_upper), alpha = 0.12, linewidth = 0) +
  ...
  labs(
    ...
    title = "ROC Curves on Test Set with 95% CI Bands",
    color = "Model"
  ) +
...
# Pick one canonical set of classes (best: from training target)
cls <- levels(train_data$diagnosis_multi2)
cls <- sort(unique(c(as.character(pred_multi), as.character(test_data$diagnosis_multi2))))


pred_multi_f <- factor(pred_multi, levels = cls)
ref_multi_f  <- factor(test_data$diagnosis_multi2, levels = cls)

cm_multi <- caret::confusionMatrix(data = pred_multi_f, reference = ref_multi_f)
...
multi_acc_ci <- c(
  estimate = as.numeric(cm_multi$overall["Accuracy"]),
  lower = as.numeric(cm_multi$overall["AccuracyLower"]),
  upper = as.numeric(cm_multi$overall["AccuracyUpper"])
)
print(paste(
  "Multi-class accuracy with 95% CI:",
  format_ci_text(multi_acc_ci["estimate"], multi_acc_ci["lower"], multi_acc_ci["upper"])
))
...
threshold_analysis <- map_dfr(thresholds, ~{
  ...
  counts_thresh <- extract_binary_counts(cm_thresh$table, positive = "GBM")
  
  sens_ci <- exact_binom_ci(counts_thresh["tp"], counts_thresh["tp"] + counts_thresh["fn"])
  spec_ci <- exact_binom_ci(counts_thresh["tn"], counts_thresh["tn"] + counts_thresh["fp"])
  ppv_ci <- exact_binom_ci(counts_thresh["tp"], counts_thresh["tp"] + counts_thresh["fp"])
  npv_ci <- exact_binom_ci(counts_thresh["tn"], counts_thresh["tn"] + counts_thresh["fn"])
  
  tibble(
    threshold = threshold,
    sensitivity = as.numeric(sens_ci["estimate"]),
    sensitivity_lower = as.numeric(sens_ci["lower"]),
    sensitivity_upper = as.numeric(sens_ci["upper"]),
    specificity = as.numeric(spec_ci["estimate"]),
    specificity_lower = as.numeric(spec_ci["lower"]),
    specificity_upper = as.numeric(spec_ci["upper"]),
    ppv = as.numeric(ppv_ci["estimate"]),
    ppv_lower = as.numeric(ppv_ci["lower"]),
    ppv_upper = as.numeric(ppv_ci["upper"]),
    npv = as.numeric(npv_ci["estimate"]),
    npv_lower = as.numeric(npv_ci["lower"]),
    npv_upper = as.numeric(npv_ci["upper"])
  ) %>%
    dplyr::mutate(
      sensitivity_CI95 = format_ci_text(sensitivity, sensitivity_lower, sensitivity_upper),
      specificity_CI95 = format_ci_text(specificity, specificity_lower, specificity_upper),
      ppv_CI95 = format_ci_text(ppv, ppv_lower, ppv_upper),
      npv_CI95 = format_ci_text(npv, npv_lower, npv_upper)
    )
})
  ...
  cohort_performance <- map_dfr(unique(test_data$cohort), ~{
    ...
    if(nrow(cohort_data) > 5) {  # Only analyze cohorts with sufficient samples
      ...
      if(length(unique(actual_cohort)) == 2) {  # Only if both classes present
        roc_cohort <- roc(actual_cohort, pred_cohort$GBM, quiet = TRUE)
        auc_cohort <- as.numeric(auc(roc_cohort))
        auc_ci <- safe_ci_auc(roc_cohort, conf.level = 0.95)
      } else {
        auc_cohort <- NA
        auc_ci <- c(lower = NA_real_, estimate = NA_real_, upper = NA_real_)
      }
      ...
      tibble(
        ...
        dplyr::mutate(
          auc_CI95 = format_ci_text(auc, auc_lower, auc_upper)
        )
    } else {
      tibble(
        cohort = .x,
...
cat(sprintf("- Non-GBM cases: %d (%.1f%%)\n", 
    sum(df_complete$is_GBM == "NonGBM"), 
    100 * mean(df_complete$is_GBM == "NonGBM")))
...
cat(sprintf("- Model: %s\n", best_model_name))
cat(sprintf(
  "- Test AUC: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$auc["estimate"]),
  as.numeric(best_metrics$auc["lower"]),
  as.numeric(best_metrics$auc["upper"])
))
cat(sprintf(
  "- Accuracy: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$accuracy["estimate"]),
  as.numeric(best_metrics$accuracy["lower"]),
  as.numeric(best_metrics$accuracy["upper"])
))
cat(sprintf(
  "- Sensitivity: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$sensitivity["estimate"]),
  as.numeric(best_metrics$sensitivity["lower"]),
  as.numeric(best_metrics$sensitivity["upper"])
))
cat(sprintf(
  "- Specificity: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$specificity["estimate"]),
  as.numeric(best_metrics$specificity["lower"]),
  as.numeric(best_metrics$specificity["upper"])
))
cat(sprintf(
  "- PPV: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$ppv["estimate"]),
  as.numeric(best_metrics$ppv["lower"]),
  as.numeric(best_metrics$ppv["upper"])
))
cat(sprintf(
  "- NPV: %.3f (95%% CI %.3f-%.3f)\n",
  as.numeric(best_metrics$npv["estimate"]),
  as.numeric(best_metrics$npv["lower"]),
  as.numeric(best_metrics$npv["upper"])
))
cat(sprintf(
  "- F1: %.3f (95%% CI %.3f-%.3f)\n\n",
  as.numeric(best_metrics$f1["estimate"]),
  as.numeric(best_metrics$f1["lower"]),
  as.numeric(best_metrics$f1["upper"])
))
